...
def insert_score(name, score, level):
    conn = psycopg2.connect(dbname='scores', user='postgres', password='chessy228', host='localhost', port='5432')
    cur = conn.cursor()
...
def get_random_food():
    global food_counter
    while True:
        pos = [random.randrange(0, SCREEN_WIDTH, 10), random.randrange(0, SCREEN_HEIGHT, 10)]
